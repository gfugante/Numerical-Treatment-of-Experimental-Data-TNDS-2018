...
oscillatoreForzatoSmorzato::oscillatoreForzatoSmorzato(){
	_omega0 = 0.;
	_alpha = 0.;
	_omegaf = 0.;
}

oscillatoreForzatoSmorzato::oscillatoreForzatoSmorzato(double omega0, double alpha, double omegaf)
{
	_omega0 = omega0;
	_alpha = alpha;
	_omegaf = omegaf;
...
VettoreLineare oscillatoreForzatoSmorzato::eval(double t, const VettoreLineare& x) const
{ 
	VettoreLineare osc(2);
 
	osc.setComponent(0, x.getComponent(1) ); 
	osc.setComponent( 1, ( -x.getComponent(0) * pow(_omega0,2) ) - (_alpha*x.getComponent(1)) + sin(_omegaf*t) ); 
